...
- [A.3.2 — OpenVINO GPU](../05_OpenVINO_GPU/OpenVINOGPU.md) completed — 06 uses the RemoteTensor path internally.
...
06 uses the RemoteTensor path from 05 internally. Frame 1 shows a JIT warm-up spike (~80–160 ms Inference) — expected. Frames 2+ stabilize to ~4–8 ms inference (f32, 256×256 model, Intel Xe).
...
Inside the Bokeh kernel, `if (mask[id] == BACKGROUND)` causes thread divergence. Replace it with `select()` (branchless) and measure the kernel time difference. See [Toolbox: Thread Divergence](../../05_Toolbox/13_Thread_Divergence/ThreadDivergence.md).
