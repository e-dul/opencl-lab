...
> **Local memory primer**: `__local` declares per-workgroup shared memory. Use `barrier(CLK_LOCAL_MEM_FENCE)` to synchronise before reading data filled by other work-items. See [Toolbox: Local Memory](../../05_Toolbox/LocalMemory/LocalMemory.md) for the full tiling pattern.
