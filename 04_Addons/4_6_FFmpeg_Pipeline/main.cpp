...
// ---------------------------------------------------------------------------
// VaapiInterop — VAAPI device context rebuild + extension function pointers
// ---------------------------------------------------------------------------

// Bundles everything needed for Intel/AMD CL-VAAPI zero-copy interop.
// `enabled` is false when the platform lacks the extension or init fails.
struct VaapiInterop {
    VADisplay   va_display{};
    cl::Context cl_ctx;
    clCreateFromVA_APIMediaSurfaceINTEL_fn    clCreateFromVA_surf{};
    clEnqueueAcquireVA_APIMediaSurfacesINTEL_fn clAcquireVA_surfs{};
    clEnqueueReleaseVA_APIMediaSurfacesINTEL_fn clReleaseVA_surfs{};
    bool enabled{false};
};

// Attempt to rebuild the CL context with CL_CONTEXT_VA_API_DISPLAY_INTEL and
// load the three VAAPI-sharing entry points.  On any failure the returned
// struct has enabled=false and the caller should keep the original context.
static VaapiInterop init_vaapi_interop(const cl::Platform& platform,
                                       const cl::Device&   device,
                                       const cl::Context&  fallback_ctx,
                                       AVBufferRef*        vaapi_dev_ctx)
{
    VaapiInterop v;
    v.cl_ctx = fallback_ctx;   // default: keep the original context

    if (!vaapi_dev_ctx) return v;
    ...
    cl_context_properties props[] = {
        ...
        CL_CONTEXT_VA_API_DISPLAY_INTEL,(cl_context_properties)v.va_display,
        CL_CONTEXT_INTEROP_USER_SYNC,   CL_FALSE,
        0
    };
    cl_int err = CL_SUCCESS;
    cl_context raw = clCreateContext(props, 1, &device(), nullptr, nullptr, &err);
    if (err != CL_SUCCESS) {
        std::cout << "[INFO] CL-VAAPI interop context failed (err=" << err
                  << "); using copy path.\n";
        return v;   // enabled stays false
    }
    ...
    v.clCreateFromVA_surf = reinterpret_cast<clCreateFromVA_APIMediaSurfaceINTEL_fn>(
        clGetExtensionFunctionAddressForPlatform(
            platform(), "clCreateFromVA_APIMediaSurfaceINTEL"));
    v.clAcquireVA_surfs = reinterpret_cast<clEnqueueAcquireVA_APIMediaSurfacesINTEL_fn>(
        clGetExtensionFunctionAddressForPlatform(
            platform(), "clEnqueueAcquireVA_APIMediaSurfacesINTEL"));
    v.clReleaseVA_surfs = reinterpret_cast<clEnqueueReleaseVA_APIMediaSurfacesINTEL_fn>(
        clGetExtensionFunctionAddressForPlatform(
            platform(), "clEnqueueReleaseVA_APIMediaSurfacesINTEL"));

    if (!v.clCreateFromVA_surf || !v.clAcquireVA_surfs || !v.clReleaseVA_surfs) {
        std::cout << "[INFO] VAAPI sharing entry points not found; "
                     "falling back to copy path.\n";
        v.cl_ctx = fallback_ctx;   // revert to original context
        return v;
    }
...
// ---------------------------------------------------------------------------
// DecoderCtx — FFmpeg decoder with optional HW acceleration
// ---------------------------------------------------------------------------
...
// Select the best available decoder for `stream`, attaching VAAPI or CUDA hw
// acceleration when `vaapi_dev_ctx` is non-null or cuvid is available.
// Falls back silently to SW on any HW open failure.
static DecoderCtx open_decoder(AVStream* stream, AVBufferRef* vaapi_dev_ctx)
{
    AVCodecParameters* codecpar = stream->codecpar;
    DecoderCtx d;

    // Decoder name selection: VAAPI wraps the standard codec; NVDEC uses named cuvid.
...
// ---------------------------------------------------------------------------
// EncoderCtx — FFmpeg encoder with optional VAAPI/SW acceleration
// ---------------------------------------------------------------------------

struct EncoderCtx {
    AVCodecContext* ctx{};
    AVBufferRef*    hw_frames_ctx{};   // non-null only for VAAPI path
    AVPixelFormat   sw_pix_fmt{AV_PIX_FMT_NV12};
    bool            using_vaapi{false};
    bool            using_nvenc{false};
};

// Select the best H.264 encoder, configure HW frames context for VAAPI, and
// open the codec.  Falls back to libx264/h264 on VAAPI open failure.
static EncoderCtx open_encoder(AVStream*    out_stream,
                                AVFormatContext* out_ctx_fmt,
                                AVBufferRef* vaapi_dev_ctx,
                                int width, int height)
{
    ...
    enc_ctx->pix_fmt      = e.using_vaapi ? AV_PIX_FMT_VAAPI : AV_PIX_FMT_YUV420P;
    ...
    if (e.using_vaapi) {
        ...
        e.hw_frames_ctx = av_buffer_ref(hw_frames_ref);
        av_buffer_unref(&hw_frames_ref);
    }

    if (out_ctx_fmt->oformat->flags & AVFMT_GLOBALHEADER)
    ...
    if (avcodec_open2(enc_ctx, encoder, nullptr) < 0) {
        if (e.using_vaapi) {
            ...
            if (e.hw_frames_ctx) { av_buffer_unref(&e.hw_frames_ctx); e.hw_frames_ctx = nullptr; }
            e.using_vaapi = false;
            encoder = avcodec_find_encoder_by_name("libx264");
            if (!encoder) encoder = avcodec_find_encoder(AV_CODEC_ID_H264);
    ...
    std::cout << "[INFO] Encoder: " << encoder->name
              << (e.using_vaapi ? " (hardware/vaapi)"
                 : e.using_nvenc ? " (hardware/nvenc)"
                 : " (software)") << "\n";
...
// ---------------------------------------------------------------------------
// map_frame_to_cl — VAAPI surface / YUV → RGBA cl::Image2D
// ---------------------------------------------------------------------------

// Converts one decoded frame into cl_src (RGBA).
// Zero-copy path: import VAAPI surface directly into CL, run nv12_to_rgba on GPU.
// GPU-assisted copy path: av_hwframe_transfer_data → upload NV12 planes → nv12_to_rgba.
// CPU fallback: sws_scale YUV420P → RGBA → enqueueWriteImage.
// Returns wall-clock elapsed ms (map stage timing).
static double map_frame_to_cl(
    AVFrame*          dec_frame,
    cl::Image2D&      cl_src,
    cl::Image2D&      cl_nv12_y,
    cl::Image2D&      cl_nv12_uv,
    cl::Kernel&       nv12_kernel,
    cl::CommandQueue& prof_queue,
    const VaapiInterop& vaapi,
    SwsContext*&      sws_ctx,          // lazy-init; modified on first call
    std::vector<uint8_t>& rgba_buf,
    int width, int height)
{
    const std::array<size_t, 3> img_origin = {0, 0, 0};
    const std::array<size_t, 3> img_region = {static_cast<size_t>(width),
                                               static_cast<size_t>(height), 1};
    const std::array<size_t, 3> uv_region  = {static_cast<size_t>(width / 2),
                                               static_cast<size_t>(height / 2), 1};

    auto t_start = Clock::now();

    if (vaapi.enabled && dec_frame->format == AV_PIX_FMT_VAAPI) {
        // ── Zero-copy path ───────────────────────────────────────────────────
        ...
        cl_int err = CL_SUCCESS;
        cl_mem y_mem = vaapi.clCreateFromVA_surf(
            vaapi.cl_ctx(), CL_MEM_READ_ONLY, &va_surf, Y_PLANE, &err);
        if (err != CL_SUCCESS)
            throw std::runtime_error("clCreateFromVA_surf(Y) failed: err=" + std::to_string(err));
        cl_mem uv_mem = vaapi.clCreateFromVA_surf(
            vaapi.cl_ctx(), CL_MEM_READ_ONLY, &va_surf, UV_PLANE, &err);
        if (err != CL_SUCCESS) {
            clReleaseMemObject(y_mem);
            throw std::runtime_error("clCreateFromVA_surf(UV) failed: err=" + std::to_string(err));
        }

        cl_mem mems[2] = {y_mem, uv_mem};
        CL_CHECK(vaapi.clAcquireVA_surfs(prof_queue(), 2, mems, 0, nullptr, nullptr));
        ...
        CL_CHECK(nv12_kernel.setArg(3, width));
        CL_CHECK(nv12_kernel.setArg(4, height));

        CL_CHECK(prof_queue.enqueueNDRangeKernel(
            nv12_kernel, cl::NullRange,
            cl::NDRange(static_cast<size_t>(width),
                        static_cast<size_t>(height)),
            cl::NullRange));
        CL_CHECK(prof_queue.finish());

        CL_CHECK(vaapi.clReleaseVA_surfs(prof_queue(), 2, mems, 0, nullptr, nullptr));
        clReleaseMemObject(y_mem);
        ...
        if (sw_frame->format == AV_PIX_FMT_NV12) {
            ...
            uint8_t* dst_data[4]   = {rgba_buf.data(), nullptr, nullptr, nullptr};
            int      dst_stride[4] = {width * 4, 0, 0, 0};
            sws_scale(sws_ctx,
                      sw_frame->data, sw_frame->linesize, 0, height,
                      dst_data, dst_stride);
            CL_CHECK(prof_queue.enqueueWriteImage(
    ...
    return Ms(Clock::now() - t_start).count();
}

// ---------------------------------------------------------------------------
// encode_cl_to_frame — cl_dst (RGBA) → AVFrame ready for avcodec_send_frame
// ---------------------------------------------------------------------------

// Converts the filtered RGBA image in cl_dst to an NV12 AVFrame.
// Zero-copy path (hw_interop && enc_using_vaapi): write directly into a VAAPI
//   surface via CL, skip CPU readback entirely.
// GPU-assisted path: rgba_to_nv12 kernels → enqueueReadImage → enc_frame.
//   If enc_using_vaapi without interop: additionally uploads via av_hwframe_transfer_data.
// Returns wall-clock elapsed ms (encode-prep timing).
// `out_send_frame` is set to the frame pointer the caller should pass to avcodec_send_frame.
// `out_hw_frame`   is non-null when a VAAPI hw_frame was allocated and must be freed by caller.
static double encode_cl_to_frame(
    cl::Image2D&      cl_dst,
    cl::Image2D&      cl_nv12_y,
    cl::Image2D&      cl_nv12_uv,
    AVFrame*          enc_frame,
    ...
    bool              enc_using_vaapi,
    int               frame_idx,
    int               width, int height,
    AVFrame**         out_send_frame,
    AVFrame**         out_hw_frame)
{
    ...
    *out_send_frame = nullptr;
    *out_hw_frame   = nullptr;

    if (vaapi.enabled && enc_using_vaapi) {
        ...
        cl_int err = CL_SUCCESS;
        cl_mem enc_y_mem  = vaapi.clCreateFromVA_surf(
            vaapi.cl_ctx(), CL_MEM_WRITE_ONLY, &enc_surf, Y_PLANE, &err);
        if (err != CL_SUCCESS)
            throw std::runtime_error("clCreateFromVA_surf(enc Y) err=" + std::to_string(err));
        cl_mem enc_uv_mem = vaapi.clCreateFromVA_surf(
            vaapi.cl_ctx(), CL_MEM_WRITE_ONLY, &enc_surf, UV_PLANE, &err);
        if (err != CL_SUCCESS) {
        ...
        CL_CHECK(vaapi.clAcquireVA_surfs(prof_queue(), 2, enc_mems, 0, nullptr, nullptr));
        ...
        CL_CHECK(rgba_nv12_y_kernel.setArg(2, width));
        CL_CHECK(rgba_nv12_y_kernel.setArg(3, height));
        CL_CHECK(prof_queue.enqueueNDRangeKernel(
        ...
        CL_CHECK(rgba_nv12_uv_kernel.setArg(2, width));
        CL_CHECK(rgba_nv12_uv_kernel.setArg(3, height));
        CL_CHECK(prof_queue.enqueueNDRangeKernel(
            rgba_nv12_uv_kernel, cl::NullRange,
            cl::NDRange(static_cast<size_t>((width + 1) / 2),
                        static_cast<size_t>((height + 1) / 2)),
            cl::NullRange));

        CL_CHECK(prof_queue.finish());

        CL_CHECK(vaapi.clReleaseVA_surfs(prof_queue(), 2, enc_mems, 0, nullptr, nullptr));
        clReleaseMemObject(enc_y_mem);
        clReleaseMemObject(enc_uv_mem);

        hw_frame->pts = static_cast<int64_t>(frame_idx);
        *out_send_frame = hw_frame;
...
static int run(int argc, char** argv)
{
    ...
    // ── Hardware interop detection + context rebuild + fn pointer loading ─────
    bool hw_interop = vaapi_dev_ctx &&
                      device_has_ext(ocl.device, "cl_intel_va_api_media_sharing");

    VaapiInterop vaapi;
    if (hw_interop) {
        vaapi = init_vaapi_interop(ocl.platform, ocl.device, ocl.context, vaapi_dev_ctx);
        hw_interop = vaapi.enabled;
        if (vaapi.enabled)
            ocl.context = vaapi.cl_ctx;   // subsequent CL objects use the interop context
    }
    ...
    // ── Decoder ──────────────────────────────────────────────────────────────
    DecoderCtx dec = open_decoder(video_stream, vaapi_dev_ctx);
    AVCodecContext* dec_ctx  = dec.ctx;
    bool            using_hw = dec.hw;
    ...
    // ── Encoder ──────────────────────────────────────────────────────────────
    EncoderCtx enc = open_encoder(out_stream, out_ctx, vaapi_dev_ctx, frame_w, frame_h);
    AVCodecContext* enc_ctx         = enc.ctx;
    bool            enc_using_vaapi = enc.using_vaapi;
    ...
    // ── Dispatch lambda: set kernel args + enqueue in one call ────────────────
    // WHY lambda: the filter kernel dispatch pattern (set 4 args, enqueue, event)
    // repeats identically for every frame; the lambda eliminates the repetition
    // without forcing a new function boundary that would break context locality.
    auto dispatch = [&](cl::Kernel& k,
                        cl::Memory  arg0, cl::Memory arg1,
                        int w, int h,
                        cl::Event& ev) {
        CL_CHECK(k.setArg(0, arg0));
        CL_CHECK(k.setArg(1, arg1));
        CL_CHECK(k.setArg(2, w));
        CL_CHECK(k.setArg(3, h));
        CL_CHECK(prof_queue.enqueueNDRangeKernel(
            k, cl::NullRange,
    ...
    for (int attempt = 0; attempt < 2; ++attempt) {
        if (attempt == 1) {
            ...
            const AVCodec* sw_dec = avcodec_find_decoder(codecpar->codec_id);
            using_hw = false;
            dec_ctx  = avcodec_alloc_context3(sw_dec);
            if (avcodec_parameters_to_context(dec_ctx, codecpar) < 0)
                throw std::runtime_error("avcodec_parameters_to_context failed (SW retry)");
            if (avcodec_open2(dec_ctx, sw_dec, nullptr) < 0)
    ...
    while (av_read_frame(fmt_ctx, pkt) >= 0) {
        ...
        while (avcodec_receive_frame(dec_ctx, dec_frame) == 0) {
            ...
            double map_ms = map_frame_to_cl(
                dec_frame, cl_src, cl_nv12_y, cl_nv12_uv,
                nv12_kernel, prof_queue, vaapi,
                sws_to_rgba, rgba_in, frame_w, frame_h);

            // ── Filter: dispatch kernel, time via cl::Event ─────────────────
            cl::Event filter_event;
            dispatch(kernel, cl_src, cl_dst, frame_w, frame_h, filter_event);
            CL_CHECK(prof_queue.finish());
            ...
            encode_cl_to_frame(
                cl_dst, cl_nv12_y, cl_nv12_uv,
                enc_frame, rgba_nv12_y_kernel, rgba_nv12_uv_kernel,
                prof_queue, vaapi, enc_ctx, enc_using_vaapi,
