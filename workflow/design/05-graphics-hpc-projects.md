...
**Version:** 1.8
**Changelog:** v1.8 — Phase 6 cleanup completed (Task 034): extracted `opencl_lab_optional_gl_interop` and `opencl_lab_fetch_tinyobjloader` macros into `common/common.cmake`; created `common/graphics_hpc_utils.hpp` (round_up, get_binary_dir, build_program, save_framebuffer) and `common/bvh_utils.hpp` (Aabb, TriangleCpu, BvhNode, BvhTree, five math helpers); legacy empty scaffolding dirs removed; all subprojects verified standalone-buildable. v1.7 — Phase 5 B4 Device Enqueue completed (Task 033). v1.6 — B4 default scene changed to `cornell_box.obj`; `builtin:spheres` fallback added for B4 reflection validation (Task 033 planning). v1.5 — Phase 4 B3 Dynamic Scene completed (Task 032); v1.4 — Phase 4 adds `--max-depth` CLI arg to B3_Dynamic for BVH depth/performance tradeoff experiments; v1.3 — Phase 4 B3 Challenge moved to new directory `B3_Ray_Tracer_BVH_Dynamic/` (Snapshots over Branches, master_specs §2); v1.2 — EGL fallback for Intel NEO gl_sharing (Task 030); v1.1 — inverted GL interop flag, B3 interactive camera, B4 exit-code fix.
**Status:** Complete — all phases done (Phase 6 cleanup completed Task 034)
...
- [x] Phase 6: Module review and cleanup — extracted `round_up`, `get_binary_dir`, `build_program`, `save_framebuffer` into `common/graphics_hpc_utils.hpp`; `Aabb`, `TriangleCpu`, `BvhNode`, `BvhTree`, and math helpers into `common/bvh_utils.hpp`; GL interop and tinyobjloader CMake blocks consolidated into `common/common.cmake` macros; legacy empty scaffolding dirs removed; all subprojects verified standalone-buildable.
...
8. **Shared Utilities Extracted to `common/` (Phase 6)**
   - **Why**: `round_up`, `get_binary_dir`, `build_program`, `save_framebuffer` and the BVH type definitions were copy-pasted across four subprojects. A single canonical version in `common/` ensures future fixes propagate everywhere. Headers are opt-in (`#include` explicit in each `main.cpp`) — not silently injected.
...
- **MANUAL: Live GL Window Verification Pending (Phase 6)**: The headless path was verified for all subprojects after macro extraction. Live GL window rendering (B2 or B3) requires a display-connected machine with `cl_khr_gl_sharing` support — not verified in CI. Flag as MANUAL in any future task that touches the GL interop path.
...
- **Directory Structure**:
  ...
  │   ├── CMakeLists.txt
  │   ├── main.cpp
  │   ├── bvh_builder.hpp        (CPU SAH-BVH, header-only; types/math from common/bvh_utils.hpp)
  ...
  ```
- **Shared Utilities** (added Phase 6):
  - `common/graphics_hpc_utils.hpp`: `round_up`, `get_binary_dir`, `build_program`, `save_framebuffer`. Opt-in via explicit `#include`.
  - `common/bvh_utils.hpp`: `Aabb`, `TriangleCpu`, `BvhNode`, `BvhTree`, `sub3`, `cross3`, `dot3`, `safe_rcp`, `moller_trumbore`. Opt-in via explicit `#include` in each `bvh_builder.hpp`.
...
- **Tooling** (module-specific additions to master_specs):
  ...
  - GLFW + OpenGL: `find_package(glfw3)` + `find_package(OpenGL)`, gated on availability; encapsulated in `opencl_lab_optional_gl_interop(<target>)` macro in `common/common.cmake`.
  - tinyobjloader: fetched via `opencl_lab_fetch_tinyobjloader(<target>)` macro in `common/common.cmake` (used by B3, B3_Dynamic, B4).
