...
- [ ] `cmake -B build && cmake --build build` from `99_Toolbox/AsyncMultiThread/` succeeds with zero errors and zero warnings on the CI platform.
- [ ] `./build/async_demo --mode basic_sync` prints a structured timing table with Upload / Kernel / Download / Total columns in ms to 3 decimal places.
...
- [ ] `./build/async_demo --mode multi_thread` runs without data races (`std::thread` + per-thread queues); no `std::mutex` on `cl::CommandQueue`.
...
- [ ] If `CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE` is unavailable, a human-readable warning is printed and the tool does not crash or hang.
- [ ] `./build/async_demo --help` prints CLI11-generated usage listing all three flags.
- [ ] No BMP file is produced (numeric pipeline tool per spec).
- [ ] `kernels/pipeline_kernel.cl` is present in `$<TARGET_FILE_DIR:async_demo>/kernels/` after build (post-build copy rule verified).
...
### Remaining
- [ ] [Remaining items after partial execution]
