...
| `raw_bayer_4k.raw` | Raw RGGB Bayer (uchar) | 3840 × 2160 | 8.0 MB | RGGB Bayer mosaic derived from `rgb_4k.bmp`. Generated by applying channel selection per pixel position: R at (even row, even col), Gr at (even row, odd col), Gb at (odd row, even col), B at (odd row, odd col). Used by Add-on 4.7 SoftISP (`--input assets/raw_bayer_4k.raw --width 3840 --height 2160`). File carries no header; dimensions must be passed explicitly at runtime. |
