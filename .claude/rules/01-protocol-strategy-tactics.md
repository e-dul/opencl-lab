...
- **Template:** See `workflow/templates/task_doc_template.md`
...
3.  **EXECUTE (Coder Mode)**
    ...
    - Do not modify `workflow/design/` files during this phase.
...
If Code contradicts Design (`workflow/design/*.md`):
...
- **Evolution (Small Change):**
    1. Update `workflow/design/` first.
    2. Create a new Task to align code.
- **Pivot (Major Change):**
    1. Re-write `workflow/design/` completely (do not just append).
    2. Move conflicting/old tasks to workflow/tasks/archive/legacy/`.
...
1.  **STRATEGY (`workflow/design/*.md`)** = **The Law (Source of Truth)**.
...
3.  **TACTICS (`workflow/tasks/*.md`)** = **The Work Order (Execution)**.
