...
## Instructions
1. **Gather Context**: Read `workflow/design/00-executive-summary.md` and `rules/00_master_specs.md` to understand the project philosophy and the specific module's goals.
2. **Fetch Template**: Read the `workflow/templates/design_doc_template.md` file from the workspace to get the exact markdown structure required.
3. **Draft the Design**: Fill out the template for the requested module. 
4. **Enforce Rules**: 
...
5. **Save**: Write the final output to `workflow/design/design_[module_name].md`.
